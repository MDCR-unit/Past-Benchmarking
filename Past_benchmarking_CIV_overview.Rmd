---
title: "Past Benchmarking - Cote d'Ivoire"
author: "OECD Development Centre - MDCR Unit"
date: "17-November-2020"
output: html_document
---

This page is a collection of the results for the Multi-dimensional Country Review's Past Benchmarking algorithm for the country **Cote d'Ivoire**. <br>

This algorithm finds an indicator's present day value for Cote d'Ivoire and searches for countries with similar levels in the past. Countries who have been at or near the same value become Benchmark Candidate Countries, which are then filtered further by a "goal" each of these countries have met. This goal is explicitly defined for each indicator and is adjusted by the application of the algorithm. The Candidate Countries are then summarized into a data table with information such as matching year, length of time to reach goal and other relevant information. A series of time series graphs for each of the Candidate Countries is also included to evaluate sustained performance over time. <br>

The uses of this algorithm are multi-faceted. Seeing how other, comparable countries have developed over time for an indicator can help us find stories in the past, provide reference points for effective policies and guide feasible goal-setting. 
<br>

The following indicators are currently available: <p>
* Constant GDP per capita (2010 USD) <br>
* Employment to population (%) <br>
* Literacy rate (%) <br>
* Mean years of schooling <br>
* Percent of population without schooling <br>
* Economic complexity index <br>
* Output per worker (GDP constant 2010 USD) <br>

The summary of the results for Cote d'Ivoire for each of these indicators are found below. 

---

#### **Indicator:** Constant GDP per capita (2010 USD)
##### **Source:** WDI

**2019 Constant GDP per capita in Cote d'Ivoire :** 1 735.80 USD <br>
**Goal :** Double (2x)

##### **Map of comparison countries**

Below is a map populated with each country that met the matching criteria. 

```{r , message=FALSE, class.source="bg-success", echo=FALSE}

library(maps)
library(rbokeh)
library(readxl)
library(dplyr)

caps<-read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/0_Indicator_Tables/map_figures.xlsx")
gdp <- read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/0_Indicator_Tables/2019_CIV_gdp.xlsx")
names(gdp)[2]<-"country"

clist <- gdp$country
gdp$`Match value`<-round(gdp$`Match value`, digits=2)

caps<-subset(caps, country %in% clist)
info.cols<-gdp[,c(2,4,5,7)]
caps<-left_join(info.cols,caps)

colnames(caps)[1]<-"Country"

figure(width = 800, height = 450, padding_factor = 0) %>%
  ly_map("world", col = "gray", line_color="black",line_alpha=0.5,fill_alpha=0.75) %>%
  ly_points(long, lat, data = caps, size = 10,
            hover = c(Country,`Match value`,`Value at goal`,
                      `Years to reach goal`,
                      Employment,Population))

```

##### **Table output of comparison countries**

This table is a summary of the results of the algorithm.

```{r , message=FALSE, class.source="bg-success", echo=FALSE, figures-side, fig.show="hold", out.width="50%"} 

library(readxl)
library(ggplot2)
library(foreach)
library(dplyr)
library(DT)

# Upload excel of output of GDP function through changing file path
gdp.output <- read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/0_Indicator_Tables/2019_CIV_gdp.xlsx")

# Data cleaning
gdp.output <- gdp.output[,-1]
gdp.output$`Match value`<-round(gdp$`Match value`, digits=2)
gdp.output$`Value at goal`<-round(gdp$`Value at goal`, digits=2)

# Data table of output
DT::datatable(gdp.output, rownames = FALSE)

```

##### **Time series of comparison countries**

These graphs show the full time series for the indicator, constant GDP per capita. The vertical dashed lines show the time window from which the country indicator value matched current day Cote d'Ivoire until it surpasses its goal - doubling in this case. The dotted line is the time series for the country of interest, Cote d'Ivoire. 

```{r , message=FALSE, class.source="bg-success", echo=FALSE, fig.show="hold", out.width="50%"} 
# Upload indicator database & output table
ind.db<-read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/indicator.db.xlsx")
gdp.output <- read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/0_Indicator_Tables/2019_CIV_gdp.xlsx")

# Parse out time series
indicator <- ind.db[,c(1:2,3)]
names(indicator)[3]<-"value"
indicator<-na.omit(indicator)

cname <-gdp.output$`Comparison country`
gdp.ts <- indicator[indicator$country %in% c("Cote d'Ivoire",cname) ,]
gdp.ts$country[gdp.ts$country=="Cote d'Ivoire"] <-"aaaCote d'Ivoire"

gdp.output <- gdp.output[order(gdp.output$`Comparison country`),]
gdp.ts <- gdp.ts[order(gdp.ts$country),]

# Set lists to create time series graphs
myear <- gdp.output$`Match year`
gyear <- gdp.output$`Year reached goal`

# Split ts by country
glist<-gdp.ts%>%
  group_split(country)

# Change COI # to the one of country of interest in glist then remove using same #
COI <- glist[[1]]
glist<-glist[-1]

# Plotting function for every country 
# **** IMPORTANT: Change here xlim to match length of time window 
ts.plotter<-function(x){
  vertical.lines <- c(myear[x],gyear[x])

  ggplot(data = glist[[x]])  + theme_linedraw()+
    geom_line(aes(y = value, x = year))+
    geom_line(data = COI, aes(y=value, x=year), linetype=3,color="black")+
    labs(title=cname[[x]], x="Year",y="Constant GDP per capita")+
    theme(plot.title = element_text(hjust = 0.5))+
    xlim(1960,2020)+
    geom_vline(xintercept=vertical.lines,
               linetype=2, colour="black")+
    annotate("rect", xmin=myear[x], xmax =gyear[x], ymin=-Inf, ymax=Inf,alpha = .1)+
    theme(axis.text = element_text(face="bold"))
}

# Plots graph for each country 
# ** IMPORTANT: Here 21 will change by # of comparison countries
gdp.plots<-foreach(x=1:23) %do%
  ts.plotter(x)

# Graph upload
# ** IMPORTANT: Here # of plots will change by # of comparison countries (obs in envi)
gdp.plots[[1]]
gdp.plots[[2]]
gdp.plots[[3]]

gdp.plots[[4]]
gdp.plots[[5]]
gdp.plots[[6]]

gdp.plots[[7]]
gdp.plots[[8]]
gdp.plots[[9]]

gdp.plots[[10]]
gdp.plots[[11]]
gdp.plots[[12]]

gdp.plots[[13]]
gdp.plots[[14]]
gdp.plots[[15]]

gdp.plots[[16]]
gdp.plots[[17]]
gdp.plots[[18]]

gdp.plots[[19]]
gdp.plots[[20]]
gdp.plots[[21]]

gdp.plots[[22]]
gdp.plots[[23]]


```





________

#### **Indicator:** Employment to population (%)
##### **Source:** WDI

**2019 Employment in Cote d'Ivoire :** 55.06% <br>
**Goal :** 1.25x

```{r , message=FALSE, class.source="bg-success", echo=FALSE}

caps<-read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/0_Indicator_Tables/map_figures.xlsx")
emp <- read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/0_Indicator_Tables/2019_CIV_employ.xlsx")
names(emp)[2]<-"country"

clist <- emp$country
emp$`Match value`<-round(emp$`Match value`, digits=2)
emp$`Value at goal`<-round(emp$`Value at goal`, digits=2)

caps<-subset(caps, country %in% clist)
info.cols<-emp[,c(2,4,5,7)]
caps<-left_join(info.cols,caps)

colnames(caps)[1]<-"Country"

figure(width = 800, height = 450, padding_factor = 0) %>%
  ly_map("world", col = "gray", line_color="black",line_alpha=0.5,fill_alpha=0.75) %>%
  ly_points(long, lat, data = caps, size = 10,
            hover = c(Country,`Match value`,`Value at goal`,
                      `Years to reach goal`,
                      Employment,Population))

```


```{r , message=FALSE, warning = FALSE, class.source="bg-success", echo=FALSE, fig.show="hold", out.width="50%"}
library(readxl)
emp.output <- read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/0_Indicator_Tables/2019_CIV_employ.xlsx")

emp.output <- emp.output[,-1]

emp.output$`Match value`<-round(emp.output$`Match value`, digits=2)
emp.output$`Value at goal`<-round(emp.output$`Value at goal`, digits=2)

DT::datatable(emp.output, rownames = FALSE)

ind.db<-read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/indicator.db.xlsx")

indicator <- ind.db[,c(1:2,4)]
names(indicator)[3]<-"value"
indicator<-na.omit(indicator)

cname <-emp.output$`Comparison country`
emp.ts <- indicator[indicator$country %in% c("Cote d'Ivoire",cname) ,]
emp.ts$country[emp.ts$country=="Cote d'Ivoire"] <-"aaaCote d'Ivoire"

emp.output <- emp.output[order(emp.output$`Comparison country`),]
emp.ts <- emp.ts[order(emp.ts$country),]

myear <- emp.output$`Match year`
gyear <- emp.output$`Year reached goal`

glist<-emp.ts%>%
  group_split(country)

COI <- glist[[1]]
glist<-glist[-1]

ts.plotter<-function(x){
  vertical.lines <- c(myear[x],gyear[x])

  ggplot(data = glist[[x]])  + theme_linedraw()+
    geom_line(aes(y = value, x = year))+
    geom_line(data = COI, aes(y=value, x=year), linetype=3,color="black")+
    labs(title=cname[[x]], x="Year",y="Employment to population (%)")+
    theme(plot.title = element_text(hjust = 0.5))+
    xlim(1991,2020)+
   geom_vline(xintercept=vertical.lines,
               linetype=2, colour="black")+
    annotate("rect", xmin=myear[x], xmax =gyear[x], ymin=-Inf, ymax=Inf,alpha = .1)+
    theme(axis.text = element_text(face="bold"))
}

emp.plots<-foreach(x=1:2) %do%
  ts.plotter(x)

emp.plots[[1]]
emp.plots[[2]]


```





________
#### **Indicator:** Literacy rate (%)
##### **Source:** WDI

**2018 literacy rate in Cote d'Ivoire :** 47.16%<br>
**Goal :**  1.5x

```{r , message=FALSE, class.source="bg-success", echo=FALSE}


caps<-read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/0_Indicator_Tables/map_figures.xlsx")
liter <- read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/0_Indicator_Tables/2019_CIV_liter.xlsx")
names(liter)[2]<-"country"

clist <- liter$country
liter$`Match value`<-round(liter$`Match value`, digits=2)

caps<-subset(caps, country %in% clist)
info.cols<-liter[,c(2,4,5,7)]
caps<-left_join(info.cols,caps)

colnames(caps)[1]<-"Country"

figure(width = 800, height = 450, padding_factor = 0) %>%
  ly_map("world", col = "gray", line_color="black",line_alpha=0.5,fill_alpha=0.75) %>%
  ly_points(long, lat, data = caps, size = 10,
            hover = c(Country,`Match value`,`Value at goal`,
                      `Years to reach goal`,
                      Employment,Population))

```


```{r , class.source="bg-success", echo=FALSE, fig.show="hold", out.width="50%", message=FALSE}
liter.output<- read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/0_Indicator_Tables/2019_CIV_liter.xlsx")

liter.output <- liter.output[,-1]

DT::datatable(liter.output, rownames = FALSE)

ind.db<-read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/indicator.db.xlsx")

indicator <- ind.db[,c(1:2,5)]
names(indicator)[3]<-"value"
indicator<-na.omit(indicator)

cname <-liter.output$`Comparison country`
liter.ts <- indicator[indicator$country %in% c("Cote d'Ivoire",cname) ,]
liter.ts$country[liter.ts$country=="Cote d'Ivoire"] <-"aaaCote d'Ivoire"

liter.output <- liter.output[order(liter.output$`Comparison country`),]
liter.ts <- liter.ts[order(liter.ts$country),]

myear <- liter.output$`Match year`
gyear <- liter.output$`Year reached goal`

glist<-liter.ts%>%
  group_split(country)

COI <- glist[[1]]
glist<-glist[-1]

ts.plotter<-function(x){
  vertical.lines <- c(myear[x],gyear[x])

  ggplot(data = glist[[x]])  + theme_linedraw()+
    geom_line(aes(y = value, x = year))+
    geom_line(data = COI, aes(y=value, x=year), linetype=3,color="black")+
    labs(title=cname[[x]], x="Year",y="Literacy rate")+
    theme(plot.title = element_text(hjust = 0.5))+
    xlim(1972,2020)+
   geom_vline(xintercept=vertical.lines,
               linetype=2, colour="black")+
    annotate("rect", xmin=myear[x], xmax =gyear[x], ymin=-Inf, ymax=Inf,alpha = .1)+
    theme(axis.text = element_text(face="bold"))
}

liter.plots<-foreach(x=1:1) %do%
  ts.plotter(x)

liter.plots[[1]]



```






________

#### **Indicator:** Mean years of schooling
##### **Source:** Barro-Lee Education Attainment dataset

**2010 mean years of schooling in Cote d'Ivoire :** 4.22 years <br>
**Goal :** Doubled (2x)


```{r , message=FALSE, class.source="bg-success", echo=FALSE}


caps<-read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/0_Indicator_Tables/map_figures.xlsx")
myr <- read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/0_Indicator_Tables/2019_CIV_meanyrs.xlsx")
names(myr)[2]<-"country"

clist <- myr$country
myr$`Match value`<-round(myr$`Match value`, digits=2)

caps<-subset(caps, country %in% clist)
info.cols<-myr[,c(2,4,5,7)]
caps<-left_join(info.cols,caps)

colnames(caps)[1]<-"Country"

figure(width = 800, height = 450, padding_factor = 0) %>%
  ly_map("world", col = "gray", line_color="black",line_alpha=0.5,fill_alpha=0.75) %>%
  ly_points(long, lat, data = caps, size = 10,
            hover = c(Country,`Match value`,`Value at goal`,
                      `Years to reach goal`,
                      Employment,Population))

```


```{r , class.source="bg-success", echo=FALSE, fig.show="hold", out.width="50%", message=FALSE}
myrs.output<- read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/0_Indicator_Tables/2019_CIV_meanyrs.xlsx")

myrs.output <- myrs.output[,-1]

DT::datatable(myrs.output, rownames = FALSE)

ind.db<-read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/indicator.db.xlsx")

indicator <- ind.db[,c(1:2,7)]
names(indicator)[3]<-"value"
indicator<-na.omit(indicator)

cname <-myrs.output$`Comparison country`
myrs.ts <- indicator[indicator$country %in% c("Cote d'Ivoire",cname) ,]
myrs.ts$country[myrs.ts$country=="Cote d'Ivoire"] <-"aaaCote d'Ivoire"

myrs.output <- myrs.output[order(myrs.output$`Comparison country`),]
myrs.ts <- myrs.ts[order(myrs.ts$country),]

myear <- myrs.output$`Match year`
gyear <- myrs.output$`Year reached goal`

glist<-myrs.ts%>%
  group_split(country)

COI <- glist[[1]]
glist<-glist[-1]

ts.plotter<-function(x){
  vertical.lines <- c(myear[x],gyear[x])

  ggplot(data = glist[[x]])  + theme_linedraw()+
    geom_line(aes(y = value, x = year))+
    geom_line(data = COI, aes(y=value, x=year), linetype=3,color="black")+
    labs(title=cname[[x]], x="Year",y="Mean years of schooling")+
    theme(plot.title = element_text(hjust = 0.5))+
    xlim(1960,2010)+
   geom_vline(xintercept=vertical.lines,
               linetype=2, colour="black")+
    annotate("rect", xmin=myear[x], xmax =gyear[x], ymin=-Inf, ymax=Inf,alpha = .1)+
    theme(axis.text = element_text(face="bold"))
}

myrs.plots<-foreach(x=1:24) %do%
  ts.plotter(x)

myrs.plots[[1]]
myrs.plots[[2]]
myrs.plots[[3]]

myrs.plots[[4]]
myrs.plots[[5]]
myrs.plots[[6]]

myrs.plots[[7]]
myrs.plots[[8]]
myrs.plots[[9]]

myrs.plots[[10]]
myrs.plots[[11]]
myrs.plots[[12]]

myrs.plots[[13]]
myrs.plots[[14]]
myrs.plots[[15]]

myrs.plots[[16]]
myrs.plots[[17]]
myrs.plots[[18]]

myrs.plots[[19]]
myrs.plots[[20]]
myrs.plots[[21]]

myrs.plots[[22]]
myrs.plots[[23]]
myrs.plots[[24]]
```





________

#### **Indicator:** Percent of population without schooling
##### **Source:** Barro-Lee Education Attainment dataset

**2010 percentage of population with no schooling in Cote d'Ivoire :**  46.95% <br>
**Goal :** Half (-0.5x)

```{r , message=FALSE, class.source="bg-success", echo=FALSE}

caps<-read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/0_Indicator_Tables/map_figures.xlsx")
nskool <- read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/0_Indicator_Tables/2019_CIV_noschool.xlsx")
names(nskool)[2]<-"country"

clist <- nskool$country
nskool$`Match value`<-round(nskool$`Match value`, digits=2)

caps<-subset(caps, country %in% clist)
info.cols<-nskool[,c(2,4,5,7)]
caps<-left_join(info.cols,caps)

colnames(caps)[1]<-"Country"

figure(width = 800, height = 450, padding_factor = 0) %>%
  ly_map("world", col = "gray", line_color="black",line_alpha=0.5,fill_alpha=0.75) %>%
  ly_points(long, lat, data = caps, size = 10,
            hover = c(Country,`Match value`,`Value at goal`,
                      `Years to reach goal`,
                      Employment,Population))

```

```{r , message=FALSE, class.source="bg-success", echo=FALSE, fig.show="hold", out.width="50%"}
noskl.output<-read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/0_Indicator_Tables/2019_CIV_noschool.xlsx")

noskl.output <- noskl.output[order(noskl.output$`Comparison country`),]
noskl.output <- noskl.output[,-1]

DT::datatable(noskl.output, rownames = FALSE)

ind.db<-read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/indicator.db.xlsx")

indicator <- ind.db[,c(1:2,6)]
names(indicator)[3]<-"value"
indicator<-na.omit(indicator)

cname <-noskl.output$`Comparison country`
noskl.ts <- indicator[indicator$country %in% c("Cote d'Ivoire",cname) ,]
noskl.ts$country[noskl.ts$country=="Cote d'Ivoire"] <-"aaaCote d'Ivoire"

noskl.output <- noskl.output[order(noskl.output$`Comparison country`),]
noskl.ts <- noskl.ts[order(noskl.ts$country),]

myear <- noskl.output$`Match year`
gyear <- noskl.output$`Year reached goal`

glist<-noskl.ts%>%
  group_split(country)

COI <- glist[[1]]
glist<-glist[-1]

ts.plotter<-function(x){
  vertical.lines <- c(myear[x],gyear[x])

  ggplot(data = glist[[x]])  + theme_linedraw()+
    geom_line(aes(y = value, x = year))+
    geom_line(data = COI, aes(y=value, x=year), linetype=3,color="black")+
    labs(title=cname[[x]], x="Year",y="Population without schooling (%)")+
    theme(plot.title = element_text(hjust = 0.5))+
    xlim(1960,2010)+
   geom_vline(xintercept=vertical.lines,
               linetype=2, colour="black")+
    annotate("rect", xmin=myear[x], xmax =gyear[x], ymin=-Inf, ymax=Inf,alpha = .1)+
    theme(axis.text = element_text(face="bold"))
}

noskl.plots<-foreach(x=1:3) %do%
  ts.plotter(x)

noskl.plots[[1]]
noskl.plots[[2]]
noskl.plots[[3]]


```





________
#### **Indicator:** Economic complexity index 
##### **Source:** OEC

**2019 economic complexity in Cote d'Ivoire :** -1.12772 <br>
**Goal :** +0.5

```{r , message=FALSE, class.source="bg-success", echo=FALSE}

caps<-read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/0_Indicator_Tables/map_figures.xlsx")
eci <- read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/0_Indicator_Tables/2019_CIV_eci.xlsx")
names(eci)[2]<-"country"

clist <- eci$country
eci$`Match value`<-round(eci$`Match value`, digits=2)

caps<-subset(caps, country %in% clist)
info.cols<-eci[,c(2,4,5,7)]
caps<-left_join(info.cols,caps)

colnames(caps)[1]<-"Country"

figure(width = 800, height = 450, padding_factor = 0) %>%
  ly_map("world", col = "gray", line_color="black",line_alpha=0.5,fill_alpha=0.75) %>%
  ly_points(long, lat, data = caps, size = 10,
            hover = c(Country,`Match value`,`Value at goal`,
                      `Years to reach goal`,
                      Employment,Population))

```

```{r , class.source="bg-success", echo=FALSE, fig.show="hold", out.width="50%", message=FALSE}
eci.output<-read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/0_Indicator_Tables/2019_CIV_eci.xlsx")

eci.output <- eci.output[,-1]

DT::datatable(eci.output, rownames = FALSE)

ind.db<-read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/indicator.db.xlsx")

indicator <- ind.db[,c(1:2,8)]
names(indicator)[3]<-"value"
indicator<-na.omit(indicator)

cname <-eci.output$`Comparison country`
eci.ts <- indicator[indicator$country %in% c("Cote d'Ivoire",cname) ,]
eci.ts$country[eci.ts$country=="Cote d'Ivoire"] <-"aaaCote d'Ivoire"


eci.output <- eci.output[order(eci.output$`Comparison country`),]
eci.ts <- eci.ts[order(eci.ts$country),]

myear <- eci.output$`Match year`
gyear <- eci.output$`Year reached goal`

glist<-eci.ts%>%
  group_split(country)

COI <- glist[[1]]
glist<-glist[-1]

ts.plotter<-function(x){
  vertical.lines <- c(myear[x],gyear[x])

  ggplot(data = glist[[x]])  + theme_linedraw()+
    geom_line(aes(y = value, x = year))+
    geom_line(data = COI, aes(y=value, x=year), linetype=3,color="black")+
    labs(title=cname[[x]], x="Year",y="Economic complexity index")+
    theme(plot.title = element_text(hjust = 0.5))+
    xlim(1964,2020)+
   geom_vline(xintercept=vertical.lines,
               linetype=2, colour="black")+
    annotate("rect", xmin=myear[x], xmax =gyear[x], ymin=-Inf, ymax=Inf,alpha = .1)+
    theme(axis.text = element_text(face="bold"))
}

eci.plots<-foreach(x=1:4) %do%
  ts.plotter(x)

eci.plots[[1]]
eci.plots[[2]]
eci.plots[[3]]

eci.plots[[4]]


```





________
#### **Indicator:** Output per worker (GDP constant 2010 USD)
##### **Source:** ILO

**2019 output per worker in Cote d'Ivoire :** 5 532.63 USD <br>
**Goal :** 2x

```{r , message=FALSE, class.source="bg-success", echo=FALSE}

caps<-read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/0_Indicator_Tables/map_figures.xlsx")
output <- read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/0_Indicator_Tables/2019_CIV_lab.xlsx")
names(output)[2]<-"country"

clist <- output$country

output$`Match value`<-round(output$`Match value`, digits=2)

caps<-subset(caps, country %in% clist)
info.cols<-output[,c(2,4,5,7)]
caps<-left_join(info.cols,caps)

colnames(caps)[c(1:4,7)]<-c("Country","Match.value","Years.to.reach.goal","Value.at.goal","Employment.rate")

figure(width = 800, height = 450, padding_factor = 0) %>%
  ly_map("world", col = "gray", line_color="black",line_alpha=0.5,fill_alpha=0.75) %>%
  ly_points(long, lat, data = caps, size = 10,
            hover = c(Country,Match.value, Value.at.goal,
                      Years.to.reach.goal,
                      Employment.rate,Population))

```

```{r , message=FALSE, class.source="bg-success", echo=FALSE, fig.show="hold", out.width="50%"}
lab.output<-read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/0_Indicator_Tables/2019_CIV_lab.xlsx")

lab.output <- lab.output[,-1]

lab.output$`Match value`<-round(lab.output$`Match value`, digits=2)
DT::datatable(lab.output, rownames = FALSE)

ind.db<-read_excel("~/Desktop/OECD/DEV-Multi-dimensional Reviews - Past Benchmarking Tool/Past_Benchmarking/indicator.db.xlsx")

indicator <- ind.db[,c(1:2,9)]
names(indicator)[3]<-"value"
indicator<-na.omit(indicator)

cname <-lab.output$`Comparison country`
lab.ts <- indicator[indicator$country %in% c("Cote d'Ivoire",cname) ,]
lab.ts$country[lab.ts$country=="Cote d'Ivoire"] <-"aaaCote d'Ivoire"

lab.output <- lab.output[order(lab.output$`Comparison country`),]
lab.ts <- lab.ts[order(lab.ts$country),]

myear <- lab.output$`Match year`
gyear <- lab.output$`Year reached goal`

glist<-lab.ts%>%
  group_split(country)

COI <- glist[[1]]
glist<-glist[-1]

ts.plotter<-function(x){
  vertical.lines <- c(myear[x],gyear[x])

  ggplot(data = glist[[x]])  + theme_linedraw()+
    geom_line(aes(y = value, x = year))+
    geom_line(data = COI, aes(y=value, x=year), linetype=3,color="black")+
    labs(title=cname[[x]], x="Year",y="Output per worker (GDP constant 2010 USD)")+
    theme(plot.title = element_text(hjust = 0.5),
          axis.text = element_text(face="bold",size=10))+
    xlim(1991,2020)+
    geom_vline(xintercept=vertical.lines,
               linetype=2, colour="black")+
    annotate("rect", xmin=myear[x], xmax =gyear[x], ymin=-Inf, ymax=Inf,alpha = .1)
}

lab.plots<-foreach(x=1:9) %do%
  ts.plotter(x)

lab.plots[[1]]
lab.plots[[2]]
lab.plots[[3]]

lab.plots[[4]]
lab.plots[[5]]
lab.plots[[6]]

lab.plots[[7]]
lab.plots[[8]]
lab.plots[[9]]


```



